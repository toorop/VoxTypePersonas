...
QtObject {
    ...
    function start() {
        ...
        if (publicKeyPath === "" || installRoot === "" || engineExecutable === ""
                || !/^(x86_64|aarch64)$/.test(architecture)
                || !/^v?[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?$/.test(releaseTag)
                || (!allowPrereleaseTest && releaseTag.includes("-"))
                || !/^https:\/\/github\.com\/toorop\/VoxTypePersonas\/releases\/download\/.+\/$/.test(downloadBaseUrl)) {
    ...
    function archiveContainsOneRegularFile() {
        ...
        return entries.length === 1 && entries[0].startsWith("-")
            && entries[0].endsWith(" voxtype-personas")
    }
