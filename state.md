...
### 2026-09-15 — Phase 12 item 4: prompt engine contract (in progress)

- Added `prompts get <id>` and `prompts set <id>` to the managed engine. Prompt content is supplied to `set` through standard input, never as a command-line argument.
- Prompt updates reuse the existing configuration validation and private atomic-write path. Empty prompts are rejected without persisting a partial change.
- Added process-level coverage for a standard-input update and exact subsequent retrieval. The remaining UI work is a multiline editor, validation guidance, and a safe test action.
- Added `prompts list` so the panel selects prompts by their stable IDs rather than a display name or inferred profile relationship.
- Added the Prompts settings UI: prompt list, multiline editor, empty-prompt guidance, and a save action. The panel sends a JSON-framed prompt through standard input so Quickshell does not need to close its input pipe; the prompt itself never appears in the process arguments.
- Validated with `cargo fmt`, 52 unit tests, 15 integration tests, QML formatting, plugin-manifest validation, and `git diff --check`. Built and deployed the development engine and `Panel.qml` to the enabled user plugin; their installed and repository panel hashes match. The user must manually restart the Omarchy shell for the UI test.
- Corrected the profile-to-prompt workflow after user review. `profiles list` now includes the actual optional prompt ID, and the Profiles management view offers `Edit prompt` for the selected non-Raw profile. It opens the linked multiline editor directly, including when a profile and its prompt use different IDs.
- Revalidated the full Rust and QML checks, then deployed the development engine, `Panel.qml`, and `BarWidget.qml` to the enabled user plugin with matching SHA-256 hashes. The user must manually restart the Omarchy shell before testing the correction.
- User UI review found the multiline editor font disproportionately large. Set it explicitly to the themed body font, matching the panel controls. Added icon labels to the back action and Settings entry to make panel navigation clearer. QML and plugin-manifest validation passed; deployed the revised `Panel.qml` with a matching SHA-256 hash. The user must manually restart the Omarchy shell before reviewing it.
- Kept the current readiness rule: editing a prompt alone does not promote a new profile from Draft, because it still lacks a configured provider and explicit model. Revisit the desired simpler activation journey with the provider-configuration work rather than creating an activatable incomplete profile.
- Installed and applied the `frontend-ui-ux` skill for a focused Profiles-screen hierarchy pass. Reorganized the existing controls without changing their engine behavior: New profile, profile selection, primary prompt editing, secondary profile details, and destructive deletion are now separate visual groups. The prompt editor is the visually distinct primary action; deletion remains confirmed and spatially separated from it.
- Validated QML formatting, the plugin manifest, and whitespace, then deployed the revised `Panel.qml` with a matching SHA-256 hash. The user must manually restart the Omarchy shell before visually validating this UI pass.
- Applied the frontend UI/UX rules consistently across every existing visible plugin surface: compact selector, settings navigation, Profiles, Prompts, Providers, update notification, installation state, and the bar-widget interaction. Internal QML helpers (`EngineInstaller.qml` and `GpgVerifier.qml`) remain non-visual and had no product UI changes.
- Replaced text-only settings navigation with keyboard-operable themed buttons; added persistent labels for profile-name and system-instruction fields; clarified empty, error, update, installation, and Providers states; and used icons only where they reinforce a labelled action. No engine contract or persistence behavior changed.
- Ran QML formatting on every QML file, plugin-manifest validation, `git diff --check`, and the Rust suite (52 unit tests and 15 integration tests). Deployed all QML files to the enabled user plugin and verified matching SHA-256 hashes. The user must manually restart the Omarchy shell before visual validation.
- Improved the settings-section affordance after user review: the selected Profiles, Prompts, or Providers button now retains the same themed fill and border as a hovered control, while still carrying its explicit selected state. The controls are also keyboard-focusable. QML formatting, plugin validation, and whitespace checks passed; deployed the updated `Panel.qml` with a matching SHA-256 hash. The user must manually restart the Omarchy shell before reviewing it.
- Fixed a focus-state collision in programmatic section navigation. Selecting `Edit prompt` changed the selected section to Prompts but could leave Profiles with keyboard focus, displaying two highlighted controls. A newly selected section now takes keyboard focus, ensuring a single visible section state. QML formatting, plugin validation, and whitespace checks passed; deployed the corrected `Panel.qml` with a matching SHA-256 hash. The user must manually restart the Omarchy shell before validating it.
- Reworked the settings information architecture after user review. Permanent navigation now contains only Profiles and Providers. A selected profile’s `Edit profile` action opens a contextual editor with an explicit return to Profiles; the editor contains its system instruction and the reserved future Model area. The standalone global Prompts surface and its unused prompt-list process were removed.
